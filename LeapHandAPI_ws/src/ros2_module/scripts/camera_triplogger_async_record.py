#!/usr/bin/env python3
# camera_triplogger_async_record.py
#
# Live camera + overlay + async recording + 1 kHz logging
# Subscribes: /sensor_vs_encoder = [sensor_raw, sensor_filt, encoder_deg]
#
# Example:
#   python3 camera_triplogger_async_record.py --cam 1 --show \
#       --record out.avi --record-fps 30 --width 1280 --height 720

import argparse, threading, queue, time, csv, math, collections, os
import numpy as np
import matplotlib
try:
...
def draw_overlay(frame, a_deg, fps_est, pts_idx, pts_mid):
    ...
    if pts_idx and pts_mid:
        cv2.line(frame, pts_idx[0], pts_idx[1], (0,255,0), 2)
        cv2.line(frame, pts_mid[0], pts_mid[1], (255,0,0), 2)
...
# ---------------- Async recording thread ----------------
def record_thread_func(writer, q: queue.Queue):
    """Write frames from queue to disk asynchronously."""
    while running or not q.empty():
        try:
            frm = q.get(timeout=0.1)
            writer.write(frm)
        except queue.Empty:
            continue
    writer.release()
...
def cam_thread(cam_id, width, height, fps, record_path, record_fps):
    ...
    writer, record_q, record_t = None, None, None
    if record_path:
        ...
        record_q = queue.Queue(maxsize=120)
        record_t = threading.Thread(target=record_thread_func, args=(writer, record_q), daemon=True)
        record_t.start()
    ...
    while running:
        ...
        # Optional: skip processing every Nth frame to maintain 30 FPS recording
        frame_skip += 1
        if frame_skip % 2 == 0:
            ...
            a_deg, pts_idx, pts_mid = float('nan'), None, None
...
# ---------------- Logger + plots + camera window ----------------
def run_logger_and_plot(csv_path, show=True):
...
def main():
    ...
    ap.add_argument('--record', default='', help='Save camera feed (e.g., out.avi or out.mp4)')
    ...
    try:
        run_logger_and_plot(args.csv, show=args.show)
    except KeyboardInterrupt:
        pass
    finally:
        global running
