...
class LeapNode(Node):
    def __init__(self):
        ...
        startup = self.declare_parameter('startup_pose', [3.14] * 16).get_parameter_value().double_array_value
        if len(startup) == 16:
            self.curr_pos = np.array(startup)
            self.get_logger().info(f"Startup pose loaded: {self.curr_pos}")
        else:
            self.get_logger().warn("Invalid startup_pose: Expected 16 values. Using default.")
            self.curr_pos = np.array([3.14] * 16)
